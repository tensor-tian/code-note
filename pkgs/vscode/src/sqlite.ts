 ...
 */
export function validateSqliteCommand(
  ...
  if (!sqliteCommand) {
    throw new Error(
      `Unable to find a valid SQLite command. Fallback binary not found.`
    );
  }
...
export function isSqliteCommandValid(sqliteCommand: string) {
  ...
  if (proc.error) {
    logger.debug(
      `'${sqliteCommand}' is not a valid SQLite command: ${proc.error}`
    );
    return false;
  }
  ...
  if (
    error &&
    !error.match(
      /\: \/lib64\/libtinfo\.so\.[0-9]+: no version information available \(required by /
    )
  ) {
  ...
  let match = output.match(
    /3\.(?:9|[0-9][0-9])\.[0-9]{1,2} [0-9]{4}\-[0-9]{2}\-[0-9]{2} [0-9]{2}\:[0-9]{2}\:[0-9]{2}/
  );

  if (!match) {
    logger.debug(
      `'${sqliteCommand}' is not a valid SQLite command: version must be >= 3.9`
    );
  }
...
export function getSqliteBinariesPath(extensionPath: string): string {
  ...
  switch (plat) {
    ...
    case "darwin":
      if (os_arch === "arm64") {
        sqliteBin = "sqlite-v3.45.1-osx-arm64";
      } else {
  ...
  if (sqliteBin) {
    ...
    if (existsSync(path)) {
      ...
      logger.debug(
        `Fallback SQLite binary not found: '${path}' does not exist.`
      );
      return "";
    }
  } else {
